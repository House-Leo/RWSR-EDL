        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0 and val_loader is not None:
                ...
                for val_data in val_loader:
                    ...
                    lpips = visuals['LPIPS']
                    avg_lpips += lpips
                    # img0 = visuals['GT'].cuda()
                    # img1 = visuals['SR'].cuda()
                    sr_img = util.tensor2img(visuals['SR'])  # uint8
                    gt_img = util.tensor2img(visuals['GT'])  # uint8

                    # img0 = lpips.im2tensor(gt_img).cuda()
                    # img1 = lpips.im2tensor(sr_img).cuda()
                    
                    # dist01 = loss_fn.forward(img0,img1).cpu().detach().numpy()

                    # LPIPS_all.append(dist01)
                ...
                # log
                logger.info('# Validation # PSNR: {:.4e} LPIPS:{:.6f}'.format(avg_psnr, avg_lpips))
                logger_val = logging.getLogger('val')  # validation logger
                # logger_val.info('<epoch:{:3d}, iter:{:8,d}> psnr: {:.4e}'.format(
                #     epoch, current_step, avg_psnr))
                logger_val.info('<epoch:{:3d}, iter:{:8,d}> psnr: {:.4e}, lpips: {:.6f}'.format(
